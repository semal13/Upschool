...
export const getGroqResponse = async (systemPrompt, messageHistoryArray = [], currentMessage, forceJson = false) => {
  ...
  if (!apiKey) {
    console.error("OpenRouter API Key bulunamadı! Lütfen .env dosyasına VITE_OPENROUTER_API_KEY ekle.");
  ...
  try {
    const res = await fetch(OPENROUTER_BASE_URL, {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://talya.app",
        "X-Title": "Talya PCOS Companion"
      },
      body: JSON.stringify({
    ...
    if (!res.ok) {
      ...
      console.error("OpenRouter HTTP hatası:", res.status, errText);
    ...
    if (!text) {
      console.warn("OpenRouter boş yanıt döndü:", data);
      return "Şu an yanıt oluşturamadım, tekrar deneyelim mi? 💜";
    }
    return text;
  } catch (error) {
    console.error("OpenRouter isteği başarısız:", error);
    return "Bir anlığına bağlantımız koptu ama ben seninleyim. Lütfen tekrar yazar mısın? 💜";
  }
};
...
const user = `DAHA ÖNCE ÖNERDİĞİN VE TEKRAR ÖNERMEMEN GEREKEN TARİFLER: ${seenRecipes.join(', ')}
...
Recipe tipleri şunlardan biri olmalı: "Glütensiz", "Yüksek Protein", "Düşük Karb", "Tümü". Workout tipleri KESİNLİKLE şunlar olmalıdır: "Düşük Efor", "Güç", "Yoga".`;
  ...
  for (const rule of extraRules) {
    ...
    if (parsed && !hasAllergenConflict(parsed, userData.allergens || [])) {
      ...
      if (updatedSeen.length > 30) updatedSeen = updatedSeen.slice(updatedSeen.length - 20);
