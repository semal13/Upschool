...
export const getGroqResponse = async (systemPrompt, messageHistoryArray = [], currentMessage, forceJson = false) => {
  ...
  if (!apiKey) {
    console.error("API Key bulunamadı! Lütfen .env dosyasına VITE_GROQ_API_KEY ekle.");
    return "Groq API anahtarı eksik. Lütfen .env dosyanı kontrol et. 💜";
  }

  const groq = new Groq({
    apiKey: apiKey,
    dangerouslyAllowBrowser: true // Frontend'den çağrılması için gereklidir
  });
